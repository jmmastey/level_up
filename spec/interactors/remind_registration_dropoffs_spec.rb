...
describe RemindRegistrationDropoffs do
  subject(:interactor) { RemindRegistrationDropoffs.new }
  ...
  it "sends email to users who haven't registered" do
    expect(subject).to receive(:lazy_users).and_return([user])
  ...
  it "finds old users with no enrollments" do
    expect(subject).to receive(:remind).with(user)
    expect(subject).not_to receive(:remind).with(new_user)
    expect(subject).not_to receive(:remind).with(enrolled_user)

    subject.call
  end

  it "sends emails only once" do
    expect(subject).to receive(:remind).with(user).once
  ...
  it "doesn't send emails for users who don't want to receive them" do
    ...
    expect(subject).not_to receive(:remind).with(optout)
